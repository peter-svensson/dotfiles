...
complete -c but-pull-all -f -a '(jq -r ".[].path" "$HOME/Library/Application Support/com.gitbutler.app/projects.json" 2>/dev/null | while read -l p; basename "$p"; basename (dirname "$p"); end | sort -u)'
